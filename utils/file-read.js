...
exports.getOldestLogTimestampFromLogs = exports.scanLogsForBlocksFirstSeen = exports.getBlockFirstSeenFromLogs = void 0;
...
function readLineAt(fd, startPos, assumeStartOfLine = false) {
...
function findTimestampPosition(fd, targetTimestamp) {
    const size = fs.fstatSync(fd).size;
    let low = 0;
    let high = size;
    let iterations = 0;
 ...
 */
function searchForFirstSeen(fd, anchor, startTimestamp, endTimestamp, hash) {
    const size = fs.fstatSync(fd).size;
...
function getBlockFirstSeenFromLogs(hash, blockTimestamp, oldestLogTimestamp) {
    ...
    try {
        ...
        logger_1.default.debug(`Cannot parse block first seen time from Core logs. Reason: ${e instanceof Error ? e.message : e}`);
        return null;
    }
    finally {
        fs.closeSync(fd);
...
exports.getBlockFirstSeenFromLogs = getBlockFirstSeenFromLogs;
function scanLogsForBlocksFirstSeen(blocks, oldestLogTimestamp) {
    const debugLogPath = config_1.default.CORE_RPC.DEBUG_LOG_PATH;
    if (!debugLogPath) {
    ...
    try {
        ...
        logger_1.default.debug(`Cannot scan blocks first seen from Core logs. Reason: ${e instanceof Error ? e.message : e}`);
    }
    return blocks.map(block => ({ hash: block.hash, firstSeen: firstSeenMap.get(block.hash) ?? null }));
}
exports.scanLogsForBlocksFirstSeen = scanLogsForBlocksFirstSeen;
function getOldestLogTimestampFromLogs(filePath) {
...
exports.getOldestLogTimestampFromLogs = getOldestLogTimestampFromLogs;
