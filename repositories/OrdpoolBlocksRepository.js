...
class OrdpoolBlocksRepository {
     ...
     */
    async getMaxStatsHeight() {
        const [rows] = await database_1.default.query(`SELECT MAX(b.height) AS h FROM ordpool_stats s JOIN blocks b ON b.hash = s.hash`);
        const h = rows[0]?.h;
     ...
     */
    async getPendingStatsCount(startHeight) {
        const [rows] = await database_1.default.query(`SELECT COUNT(*) AS c
       FROM blocks
       WHERE height >= ?
         AND NOT EXISTS (SELECT 1 FROM ordpool_stats WHERE ordpool_stats.hash = blocks.hash)
         AND NOT EXISTS (SELECT 1 FROM ordpool_stats_skipped WHERE ordpool_stats_skipped.height = blocks.height)`, [startHeight]);
