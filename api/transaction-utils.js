...
const p_limit_1 = __importDefault(require("../utils/p-limit"));
class TransactionUtils {
    ...
    async $getTransactionExtendedRetry(txid, addPrevouts = false, lazyPrevouts = false, forceCore = false, addMempoolData = false) {
        try {
            ...
            if (result) {
                ...
                logger_1.default.err(`Cannot fetch tx ${txid}. Reason: backend returned null data`);
            }
        }
        catch (e) {
            logger_1.default.err(`Cannot fetch tx ${txid}. Reason: ` + (e instanceof Error ? e.message : e));
        }
     ...
     * @asyncUnsafe
     */
    async $getTransactionExtended(txId, addPrevouts = false, lazyPrevouts = false, forceCore = false, addMempoolData = false) {
        let transaction;
        if (forceCore === true) {
            transaction = await bitcoin_api_factory_1.bitcoinCoreApi.$getRawTransaction(txId, false, addPrevouts, lazyPrevouts);
        }
        else {
            transaction = await bitcoin_api_factory_1.default.$getRawTransaction(txId, false, addPrevouts, lazyPrevouts);
        }
    ...
    /** @asyncUnsafe */
    async $getMempoolTransactionExtended(txId, addPrevouts = false, lazyPrevouts = false, forceCore = false) {
        return (await this.$getTransactionExtended(txId, addPrevouts, lazyPrevouts, forceCore, true));
    }
    /** @asyncUnsafe */
    async $getMempoolTransactionsExtended(txids, addPrevouts = false, lazyPrevouts = false, forceCore = false) {
        if (forceCore || config_1.default.MEMPOOL.BACKEND !== 'esplora') {
            const limiter = (0, p_limit_1.default)(8); // Run 8 requests at a time
            const results = await Promise.allSettled(txids.map(txid => limiter(() => this.$getMempoolTransactionExtended(txid, addPrevouts, lazyPrevouts, forceCore))));
            return results.filter(reply => reply.status === 'fulfilled')
                .map(r => r.value);
        }
        else {
            const transactions = await bitcoin_api_factory_1.default.$getMempoolTransactions(txids);
    ...
    extendMempoolTransaction(transaction) {
        const vsize = Math.ceil(transaction.weight / 4);
        const fractionalVsize = (transaction.weight / 4);
        const sigops = common_1.Common.isLiquid() ? 0 : (transaction.sigops != null ? transaction.sigops : this.countSigops(transaction));
        ...
        const transactionExtended = Object.assign(transaction, {
     ...
     */
    countScriptSigops(script, isRawScript = false, witness = false) {
        if (!script?.length) {
            return 0;
        }
        let sigops = 0;
   ...
   */
    ...
    identifyPrioritizedTransactions(transactions, rateKey) {
