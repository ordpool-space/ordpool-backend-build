...
if (worker_threads_1.parentPort) {
    worker_threads_1.parentPort.on('message', (params) => {
        ...
        if (worker_threads_1.parentPort) {
            worker_threads_1.parentPort.postMessage({ blocks, rates, clusters });
        }
    });
}
/*
...
*/
function makeBlockTemplates(mempool) {
    const start = Date.now();
    const auditPool = new Map();
    const mempoolArray = [];
    const cpfpClusters = new Map();
    mempool.forEach(tx => {
        tx.dirty = false;
        ...
        mempoolArray.push(auditPool.get(tx.uid));
    });
    ...
    const blockSigops = 0;
    let transactions = [];
    const modified = new pairing_heap_1.PairingHeap((a, b) => {
        if (a.score === b.score) {
            // tie-break by uid for stability
            return a.uid > b.uid;
        }
    ...
    while ((top < mempoolArray.length || !modified.isEmpty())) {
        ...
        if (nextTx && !nextTx?.used) {
            ...
            if (blocks.length >= 7 || ((blockWeight + nextTx.ancestorWeight < config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS) && (blockSigops + nextTx.ancestorSigops <= 80000))) {
        ...
        // this block is full
        const exceededPackageTries = failures > 1000 && blockWeight > (config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS - 4000);
    ...
    if (overflow.length > 0) {
        logger_1.default.warn('GBT overflow list unexpectedly non-empty after final block constructed');
    }
    ...
    logger_1.default.debug('Mempool templates calculated in ' + time / 1000 + ' seconds');
