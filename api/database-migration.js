...
class DatabaseMigration {
    static currentVersion = 111;
    queryTimeout = 3600_000;
    statisticsAddedIndexed = false;
    uniqueLogs = [];
    blocksTruncatedMessage = `'blocks' table has been truncated.`;
    hashratesTruncatedMessage = `'hashrates' table has been truncated.`;
    /**
     * Avoid printing multiple time the same message
     */
    uniqueLog(loggerFunction, msg) {
    ...
    async $initializeOrMigrateDatabase() {
        logger_1.default.debug('MIGRATIONS: Running migrations');
        ...
        if (!await this.$checkIfTableExists('state')) {
            logger_1.default.debug('MIGRATIONS: `state` table does not exist. Creating it.');
            try {
                await this.$createMigrationStateTable();
            }
            catch (e) {
                logger_1.default.err('MIGRATIONS: Unable to create `state` table, aborting in 10 seconds. ' + e);
                await common_1.Common.sleep$(10000);
                process.exit(-1);
            }
            logger_1.default.debug('MIGRATIONS: `state` table initialized.');
        }
        ...
        try {
            ...
            logger_1.default.err('MIGRATIONS: Unable to get current database migration version, aborting in 10 seconds. ' + e);
            await common_1.Common.sleep$(10000);
            process.exit(-1);
        }
        if (databaseSchemaVersion === 0) {
            logger_1.default.info('Initializing database (first run, clean install)');
        }
        if (databaseSchemaVersion <= 2) {
            // Disable some spam logs when they're not relevant
            this.uniqueLog(logger_1.default.notice, this.blocksTruncatedMessage);
            this.uniqueLog(logger_1.default.notice, this.hashratesTruncatedMessage);
        }
        logger_1.default.debug('MIGRATIONS: Current state.schema_version ' + databaseSchemaVersion);
        logger_1.default.debug('MIGRATIONS: Latest DatabaseMigration.version is ' + DatabaseMigration.currentVersion);
        if (databaseSchemaVersion >= DatabaseMigration.currentVersion) {
            logger_1.default.debug('MIGRATIONS: Nothing to do.');
            return;
        }
        ...
        try {
            ...
            logger_1.default.err('MIGRATIONS: Unable to create required tables, aborting in 10 seconds. ' + e);
            await common_1.Common.sleep$(10000);
            process.exit(-1);
        }
        if (DatabaseMigration.currentVersion > databaseSchemaVersion) {
            try {
                ...
                if (databaseSchemaVersion === 0) {
                    logger_1.default.notice(`MIGRATIONS: OK. Database schema has been properly initialized to version ${DatabaseMigration.currentVersion} (latest version)`);
                }
                else {
                    logger_1.default.notice(`MIGRATIONS: OK. Database schema have been migrated from version ${databaseSchemaVersion} to ${DatabaseMigration.currentVersion} (latest version)`);
                }
            }
            catch (e) {
                logger_1.default.err('MIGRATIONS: Unable to migrate database, aborting. ' + e);
            }
        }
        return;
    }
     ...
     * @asyncUnsafe
     */
    async $createMissingTablesAndIndexes(databaseSchemaVersion) {
        await this.$setStatisticsAddedIndexedFlag(databaseSchemaVersion);
        const isBitcoin = ['mainnet', 'testnet', 'signet', 'testnet4', 'regtest'].includes(config_1.default.MEMPOOL.NETWORK);
      ...
      `);
        ...
        if (databaseSchemaVersion < 26 && isBitcoin === true) {
            if (config_1.default.LIGHTNING.ENABLED) {
                this.uniqueLog(logger_1.default.notice, `'lightning_stats' table has been truncated.`);
            }
        ...
        if (databaseSchemaVersion < 28 && isBitcoin === true) {
            if (config_1.default.LIGHTNING.ENABLED) {
                this.uniqueLog(logger_1.default.notice, `'lightning_stats' and 'node_stats' tables have been truncated.`);
            }
            await this.$executeQuery(`TRUNCATE lightning_stats`);
        ...
        if (databaseSchemaVersion < 54) {
            this.uniqueLog(logger_1.default.notice, `'prices' table has been truncated`);
            await this.$executeQuery(`TRUNCATE prices`);
            if (isBitcoin === true) {
                this.uniqueLog(logger_1.default.notice, `'blocks_prices' table has been truncated`);
        ...
        if (databaseSchemaVersion < 56) {
            ...
            this.uniqueLog(logger_1.default.notice, '`pools` table has been truncated`');
        ...
        if (databaseSchemaVersion < 59 && (config_1.default.MEMPOOL.NETWORK === 'signet' || config_1.default.MEMPOOL.NETWORK === 'testnet')) {
        ...
        if (databaseSchemaVersion < 68 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
        ...
        if (databaseSchemaVersion < 69 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
        ...
        if (databaseSchemaVersion < 70 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
            await this.$executeQuery('ALTER TABLE accelerations MODIFY COLUMN added DATETIME;');
            await this.updateToSchemaVersion(70);
        }
        if (databaseSchemaVersion < 71 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
            await this.$executeQuery('TRUNCATE TABLE elements_pegs');
        ...
        if (databaseSchemaVersion < 73 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
            // Clear bad data
            await this.$executeQuery(`TRUNCATE accelerations`);
            this.uniqueLog(logger_1.default.notice, `'accelerations' table has been truncated`);
            await this.updateToSchemaVersion(73);
        }
        if (databaseSchemaVersion < 74 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
        ...
        if (databaseSchemaVersion < 77 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
        ...
        if (databaseSchemaVersion < 79 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
            // Clear bad data
            await this.$executeQuery(`TRUNCATE accelerations`);
            this.uniqueLog(logger_1.default.notice, `'accelerations' table has been truncated`);
      ...
      `);
        ...
        if (databaseSchemaVersion < 82 && isBitcoin === true && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
      ...
      `);
        ...
        if (databaseSchemaVersion < 92 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
            await this.$executeQuery(`
      ...
      `);
        ...
        if (databaseSchemaVersion < 93 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
            await this.$executeQuery(`
      ...
      `);
        ...
        `);
            ...
            if (config_1.default.MEMPOOL.NETWORK !== 'mainnet') {
        ...
        if (databaseSchemaVersion < 95) {
            ...
            const [poolJsonShaDb] = await database_1.default.query(`SELECT string FROM state WHERE name = 'pools_json_sha'`);
        ...
        if (databaseSchemaVersion < 98 && config_1.default.MEMPOOL.NETWORK === 'mainnet') {
        ...
        // (safe to make this conditional on the network since it doesn't change the database schema)
        if (databaseSchemaVersion < 105 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
            ...
            const [stateRows] = await database_1.default.query(`SELECT name, number FROM state WHERE name IN ('last_elements_block', 'last_bitcoin_block_audit')`);
            const lastElementsBlock = Number(stateRows?.find((row) => row.name === 'last_elements_block')?.number ?? 0);
            const lastBlockAudit = Number(stateRows?.find((row) => row.name === 'last_bitcoin_block_audit')?.number ?? 0);
        ...
        // (safe to make this conditional on the network since it doesn't change the database schema)
        if (databaseSchemaVersion < 106 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
        ...
        // safe to make this conditional on the network since it doesn't change the database schema
        if (databaseSchemaVersion < 109 && config_1.default.MEMPOOL.NETWORK === 'liquid') {
     ...
     */
    async $setStatisticsAddedIndexedFlag(databaseSchemaVersion) {
        if (databaseSchemaVersion >= 2) {
        ...
        WHERE table_schema=DATABASE() AND table_name='statistics' AND index_name='added';`;
            ...
            if (rows[0].hasIndex === 0) {
                logger_1.default.debug('MIGRATIONS: `statistics.added` is not indexed');
                this.statisticsAddedIndexed = false;
            }
            else if (rows[0].hasIndex === 1) {
                logger_1.default.debug('MIGRATIONS: `statistics.added` is already indexed');
                this.statisticsAddedIndexed = true;
            }
        }
        catch (e) {
            ...
            logger_1.default.err('MIGRATIONS: Unable to check if `statistics.added` INDEX exist or not.');
            this.statisticsAddedIndexed = true;
        }
    }
    /**
    ...
    async $executeQuery(query, silent = false) {
        if (!silent) {
            logger_1.default.debug('MIGRATIONS: Execute query:\n' + query);
        }
    ...
    async $checkIfTableExists(table) {
        const query = `SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = '${config_1.default.DATABASE.DATABASE}' AND TABLE_NAME = '${table}'`;
        const [rows] = await database_1.default.query({ sql: query, timeout: this.queryTimeout });
     ...
     * @asyncUnsafe
     */
    async $getSchemaVersionFromDatabase() {
     ...
     * @asyncUnsafe
     */
    async $createMigrationStateTable() {
    ...
    async $migrateTableSchemaFromVersion(version) {
        ...
        logger_1.default.notice(`MIGRATIONS: ${version > 0 ? 'Upgrading' : 'Initializing'} database schema version number to ${DatabaseMigration.currentVersion}`);
    ...
    getMigrationQueriesFromVersion(version) {
        ...
        const isBitcoin = ['mainnet', 'testnet', 'signet', 'testnet4', 'regtest'].includes(config_1.default.MEMPOOL.NETWORK);
        if (version < 1) {
            if (config_1.default.MEMPOOL.NETWORK !== 'liquid' && config_1.default.MEMPOOL.NETWORK !== 'liquidtestnet') {
                if (version > 0) {
                    logger_1.default.notice(`MIGRATIONS: Migrating (shifting) statistics table data`);
                }
    ...
    /** @asyncUnsafe */
    async updateToSchemaVersion(version) {
        await this.$executeQuery(`UPDATE state SET number = ${version} WHERE name = 'schema_version';`);
    }
    /**
     * Print current database version
     */
    async $printDatabaseVersion() {
        try {
            const [rows] = await this.$executeQuery('SELECT VERSION() as version;', true);
            logger_1.default.debug(`MIGRATIONS: Database engine version '${rows[0].version}'`);
        }
        catch (e) {
            logger_1.default.debug(`MIGRATIONS: Could not fetch database engine version. ` + e);
        }
    }
    // Couple of wrappers to clean the main logic
    getShiftStatisticsQuery() {
    ...
    getCreateFederationPegScriptsTableQuery() {
    ...
    getCreateBlocksSummariesTableQuery() {
    ...
    getCreateDifficultyAdjustmentsTableQuery() {
    ...
    getCreateLightningStatisticsQuery() {
    ...
    getCreateBlocksAuditsTableQuery() {
    ...
    getCreateBlocksPricesTableQuery() {
    ...
    getCreateLNNodesSocketsTableQuery() {
    ...
    getCreateLNNodeRecordsTableQuery() {
    ...
    getCreateTransactionsTableQuery() {
    ...
    getCreateCompactCPFPTableQuery() {
    ...
    getCreateCompactTransactionsTableQuery() {
    ...
    getCreateAccelerationsTableQuery() {
    ...
    /** @asyncUnsafe */
    async $blocksReindexingTruncate() {
        logger_1.default.warn(`Truncating pools, blocks, hashrates and difficulty_adjustments tables for re-indexing (using '--reindex-blocks'). You can cancel this command within 5 seconds`);
    ...
    async $convertCompactCpfpTables() {
        try {
            const batchSize = 250;
            const maxHeight = await BlocksRepository_1.default.$mostRecentBlockHeight() || 0;
            const [minHeightRows] = await database_1.default.query(`SELECT MIN(height) AS minHeight from cpfp_clusters`);
          ...
          `, [height, height - batchSize]);
                if (rows?.length) {
                    await CpfpRepository_1.default.$batchSaveClusters(rows.map(row => {
                        return {
                            root: row.root,
                            height: row.height,
                ...
                logger_1.default.debug(`Migrated cpfp data from block ${height} to ${height - batchSize} in ${elapsed.toFixed(2)} seconds | total elapsed: ${runningFor.toFixed(2)} seconds`);
                timer = new Date().getTime() / 1000;
                height -= batchSize;
            }
        }
        catch (e) {
            logger_1.default.warn(`Failed to migrate cpfp transaction data`);
        }
    }
    async $fixBadV1AuditBlocks() {
        const badBlocks = [
...
exports.default = new DatabaseMigration();
