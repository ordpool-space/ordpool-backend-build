...
class ChannelsApi {
    async $getAllChannels() {
        try {
            ...
            logger_1.default.err('$getAllChannels error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getAllChannelsGeo(publicKey, style) {
        try {
            let select;
            if (style === 'widget') {
                select = `
      ...
      `;
        ...
        `;
            ...
            logger_1.default.err('$getAllChannelsGeo error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $searchChannelsById(search) {
        try {
            ...
            const [rows] = await database_1.default.query(query, [searchStripped, searchStripped]);
            return rows;
        }
        catch (e) {
            logger_1.default.err('$searchChannelsById error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getChannelsByStatus(status) {
        try {
            ...
            logger_1.default.err('$getChannelsByStatus error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getClosedChannelsWithoutReason() {
        try {
            ...
            logger_1.default.err('$getClosedChannelsWithoutReason error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getPenaltyClosedChannels() {
        try {
            const query = `
      ...
      `;
            ...
            logger_1.default.err('$getPenaltyClosedChannels error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getUnresolvedClosedChannels() {
        try {
            ...
            logger_1.default.err('$getUnresolvedClosedChannels error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getChannelsWithoutSourceChecked() {
        try {
            const query = `
        SELECT channels.*
        FROM channels
    ...
    async $getChannelsWithoutCreatedDate() {
        try {
            ...
            logger_1.default.err('$getChannelsWithoutCreatedDate error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getChannel(id) {
      ...
      `;
            ...
            logger_1.default.err('$getChannel error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getChannelsStats() {
        try {
            ...
            const [avgCapacity] = await database_1.default.query(query);
            query = `SELECT capacity FROM channels WHERE status = 1 ORDER BY capacity`;
            let [capacity] = await database_1.default.query(query);
            ...
            let [feeRates1] = await database_1.default.query(query);
            ...
            let [feeRates2] = await database_1.default.query(query);
            ...
            let [baseFees1] = await database_1.default.query(query);
            ...
            let [baseFees2] = await database_1.default.query(query);
            ...
            logger_1.default.err(`Cannot calculate channels statistics. Reason: ${e instanceof Error ? e.message : e}`);
            throw e;
        }
    }
    async $getChannelsByTransactionId(transactionIds) {
        try {
            const query = `
      ...
      `;
            const [rows] = await database_1.default.query(query, [[transactionIds], [transactionIds]]);
            const channels = rows.map((row) => this.convertChannel(row));
            return channels;
        }
        catch (e) {
            logger_1.default.err('$getChannelByTransactionId error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    /** @asyncSafe */
    async $getChannelByClosingId(transactionId) {
        try {
            const query = `
        SELECT
          channels.*
      ...
      `;
            const [rows] = await database_1.default.query(query, [transactionId]);
            if (rows.length > 0) {
            ...
            logger_1.default.err('$getChannelByClosingId error: ' + (e instanceof Error ? e.message : e));
            // don't throw - this data isn't essential
        }
    }
    async $getChannelsByOpeningId(transactionId) {
        try {
            const query = `
        SELECT
          channels.*
      ...
      `;
            ...
            logger_1.default.err('$getChannelsByOpeningId error: ' + (e instanceof Error ? e.message : e));
            // don't throw - this data isn't essential
        }
    }
    /** @asyncSafe */
    async $updateClosingInfo(channelInfo) {
        try {
            const query = `
        UPDATE channels SET
      ...
      `;
            ...
            logger_1.default.err('$updateClosingInfo error: ' + (e instanceof Error ? e.message : e));
            // don't throw - this data isn't essential
        }
    }
    /** @asyncSafe */
    async $updateOpeningInfo(channelInfo) {
        try {
            const query = `
        UPDATE channels SET
      ...
      `;
            ...
            logger_1.default.err('$updateOpeningInfo error: ' + (e instanceof Error ? e.message : e));
            // don't throw - this data isn't essential
        }
    }
    async $markChannelSourceChecked(id) {
        try {
            const query = `
        UPDATE channels
      ...
      `;
            ...
            logger_1.default.err('$markChannelSourceChecked error: ' + (e instanceof Error ? e.message : e));
            // don't throw - this data isn't essential
        }
    }
    async $getChannelsForNode(public_key, index, length, status) {
        try {
      ...
      `;
            const [channelsFromNode] = await database_1.default.query(query, [public_key]);
      ...
      `;
            const [channelsToNode] = await database_1.default.query(query, [public_key]);
            ...
            for (const row of allChannels) {
                let channel;
                if (index >= 0) {
                    const activeChannelsStats = await nodes_api_1.default.$getActiveChannelsStats(row.public_key);
                    channel = {
                        status: row.status,
            ...
            logger_1.default.err('$getChannelsForNode error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getChannelsCountForNode(public_key, status) {
        try {
      ...
      `;
            const [rows] = await database_1.default.query(query, [public_key, public_key]);
            return rows[0]['count'];
        }
        catch (e) {
            logger_1.default.err('$getChannelsForNode error: ' + (e instanceof Error ? e.message : e));
     ...
     * @asyncUnsafe
     */
    async $saveChannel(channel, status = 1) {
        if (!channel.chan_point?.length) {
            return;
        }
        const [txid, vout] = channel.chan_point.split(':');
        const policy1 = channel.node1_policy || {};
        const policy2 = channel.node2_policy || {};
      ...
      ;`;
        await database_1.default.query(query, [
            common_1.Common.channelShortIdToIntegerId(channel.channel_id),
            common_1.Common.channelIntegerIdToShortId(channel.channel_id),
            channel.capacity,
            txid,
            vout,
            common_1.Common.utcDateToMysql(channel.last_update),
            channel.node1_pub,
            ...
            policy1.min_htlc,
            common_1.Common.utcDateToMysql(policy1.last_update),
            channel.node2_pub,
            ...
            policy2.min_htlc,
            common_1.Common.utcDateToMysql(policy2.last_update),
            channel.capacity,
            common_1.Common.utcDateToMysql(channel.last_update),
            channel.node1_pub,
            policy1.fee_base_msat,
     ...
     */
    async $setChannelsInactive(graphChannelsIds) {
      ...
      `);
            if (result[0].changedRows ?? 0 > 0) {
                logger_1.default.debug(`Marked ${result[0].changedRows} channels as inactive because they are not in the graph`, logger_1.default.tags.ln);
            }
        }
        catch (e) {
            logger_1.default.err('$setChannelsInactive() error: ' + (e instanceof Error ? e.message : e));
        }
    }
    /** @asyncSafe */
    async $getLatestChannelUpdateForNode(publicKey) {
        try {
            const query = `
      ...
      `;
            ...
            logger_1.default.err(`Can't getLatestChannelUpdateForNode for ${publicKey}. Reason ${e instanceof Error ? e.message : e}`);
        }
        return 0;
    }
}
exports.default = new ChannelsApi();
