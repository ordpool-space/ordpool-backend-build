...
class NodesRoutes {
    ...
    initRoutes(app) {
        app
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/world', this.$getWorldNodes)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/country/:country', this.$getNodesPerCountry)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/search/:search', this.$searchNode)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/isp-ranking', this.$getISPRanking)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/isp/:isp', this.$getNodesPerISP)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/countries', this.$getNodesCountries)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/rankings', this.$getNodesRanking)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/rankings/liquidity', this.$getTopNodesByCapacity)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/rankings/connectivity', this.$getTopNodesByChannels)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/rankings/age', this.$getOldestNodes)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/:public_key/statistics', this.$getHistoricalNodeStats)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/:public_key/fees/histogram', this.$getFeeHistogram)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/:public_key', this.$getNode)
            .get(config_1.default.MEMPOOL.API_URL_PREFIX + 'lightning/nodes/group/:name', this.$getNodeGroup);
    }
    async $searchNode(req, res) {
        try {
            const nodes = await nodes_api_1.default.$searchNodeByPublicKeyOrAlias(req.params.search);
            res.json(nodes);
        }
        catch (e) {
            (0, api_1.handleError)(req, res, 500, 'Failed to search for node');
        }
    }
    async $getNodeGroup(req, res) {
        try {
            let nodesList;
            const nodes = [];
            switch (config_1.default.MEMPOOL.NETWORK) {
                case 'testnet':
                    nodesList = [
            ...
            for (const pubKey of nodesList) {
                try {
                    const node = await nodes_api_1.default.$getNode(pubKey);
                    if (node) {
                        nodes.push(node);
                    }
                }
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get node group');
        }
    }
    async $getNode(req, res) {
        try {
            const node = await nodes_api_1.default.$getNode(req.params.public_key);
            if (!node) {
                (0, api_1.handleError)(req, res, 404, 'Node not found');
                return;
            }
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get node');
        }
    }
    async $getHistoricalNodeStats(req, res) {
        try {
            const statistics = await nodes_api_1.default.$getNodeStats(req.params.public_key);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json(statistics);
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get historical node stats');
        }
    }
    async $getFeeHistogram(req, res) {
        try {
            const node = await nodes_api_1.default.$getFeeHistogram(req.params.public_key);
            if (!node) {
                (0, api_1.handleError)(req, res, 404, 'Node not found');
                return;
            }
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get fee histogram');
        }
    }
    async $getNodesRanking(req, res) {
        try {
            const topCapacityNodes = await nodes_api_1.default.$getTopCapacityNodes(false);
            const topChannelsNodes = await nodes_api_1.default.$getTopChannelsNodes(false);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json({
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get nodes ranking');
        }
    }
    async $getTopNodesByCapacity(req, res) {
        try {
            const topCapacityNodes = await nodes_api_1.default.$getTopCapacityNodes(true);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get top nodes by capacity');
        }
    }
    async $getTopNodesByChannels(req, res) {
        try {
            const topCapacityNodes = await nodes_api_1.default.$getTopChannelsNodes(true);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json(topCapacityNodes);
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get top nodes by channels');
        }
    }
    async $getOldestNodes(req, res) {
        try {
            const topCapacityNodes = await nodes_api_1.default.$getOldestNodes(true);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json(topCapacityNodes);
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get oldest nodes');
        }
    }
    async $getISPRanking(req, res) {
        try {
            const nodesPerAs = await nodes_api_1.default.$getNodesISPRanking();
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get ISP ranking');
        }
    }
    async $getWorldNodes(req, res) {
        try {
            const worldNodes = await nodes_api_1.default.$getWorldNodes();
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 600).toUTCString());
            res.json(worldNodes);
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get world nodes');
        }
    }
    async $getNodesPerCountry(req, res) {
        try {
            const [country] = await database_1.default.query(`SELECT geo_names.id, geo_names_country.names as country_names
        FROM geo_names
        JOIN geo_names geo_names_country on geo_names.id = geo_names_country.id AND geo_names_country.type = 'country'
        WHERE geo_names.type = 'country_iso_code' AND geo_names.names = ?`, [req.params.country]);
            if (country.length === 0) {
                (0, api_1.handleError)(req, res, 404, `This country does not exist or does not host any lightning nodes on clearnet`);
                return;
            }
            const nodes = await nodes_api_1.default.$getNodesPerCountry(country[0].id);
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json({
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get nodes per country');
        }
    }
    async $getNodesPerISP(req, res) {
        try {
            const [isp] = await database_1.default.query(`SELECT geo_names.names as isp_name
        FROM geo_names
        WHERE geo_names.type = 'as_organization' AND geo_names.id = ?`, [req.params.isp]);
            if (isp.length === 0) {
                (0, api_1.handleError)(req, res, 404, `This ISP does not exist or does not host any lightning nodes on clearnet`);
                return;
            }
            const nodes = await nodes_api_1.default.$getNodesPerISP(req.params.isp || '');
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
            res.json({
            ...
            (0, api_1.handleError)(req, res, 500, 'Failed to get nodes per ISP');
        }
    }
    async $getNodesCountries(req, res) {
        try {
            const nodesPerAs = await nodes_api_1.default.$getNodesCountries();
            res.header('Pragma', 'public');
            res.header('Cache-control', 'public');
            res.setHeader('Expires', new Date(Date.now() + 1000 * 600).toUTCString());
            res.json(nodesPerAs);
...
exports.default = new NodesRoutes();
