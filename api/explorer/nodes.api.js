...
class NodesApi {
    async $getWorldNodes() {
        ...
        ORDER BY capacity
      `;
            const [nodes] = await database_1.default.query(query);
        ...
        WHERE status = 1 AND nodes.as_number IS NOT NULL
      `;
            const [maximums] = await database_1.default.query(query);
            return {
            ...
            logger_1.default.err(`Can't get world nodes list. Reason: ${e instanceof Error ? e.message : e}`);
        }
    }
    async $getNode(public_key) {
        try {
            // General info
            let query = `
        ...
        WHERE public_key = ?
      `;
            let [rows] = await database_1.default.query(query, [public_key]);
            if (rows.length === 0) {
            ...
            if (node.features) {
                ...
                node.featuresBits = (0, format_1.bin2hex)(node.featuresBits.reverse().join(''));
            }
            // Active channels and capacity
            const activeChannelsStats = await this.$getActiveChannelsStats(public_key);
      ...
      `;
            [rows] = await database_1.default.query(query, [public_key, public_key]);
      ...
      `;
        ...
        WHERE public_key = ?
      `;
            [rows] = await database_1.default.query(query, [public_key]);
            node.custom_records = {};
            ...
            logger_1.default.err(`Cannot get node information for ${public_key}. Reason: ${(e instanceof Error ? e.message : e)}`);
            throw e;
        }
    }
    /** @asyncUnsafe */
    async $getActiveChannelsStats(node_public_key) {
        const query = `
    ...
    `;
        const [rows] = await database_1.default.query(query, [node_public_key, node_public_key]);
        if (rows.length > 0) {
            return {
    ...
    async $getFeeHistogram(node_public_key) {
        try {
            const inQuery = `
        ...
        GROUP BY bucket;
      `;
            const [inRows] = await database_1.default.query(inQuery, [node_public_key, node_public_key, node_public_key, node_public_key]);
            const outQuery = `
        SELECT CASE WHEN fee_rate <= 10.0 THEN CEIL(fee_rate)
                    WHEN (fee_rate > 10.0 and fee_rate <= 100.0) THEN CEIL(fee_rate / 10.0) * 10.0
                    WHEN (fee_rate > 100.0 and fee_rate <= 1000.0) THEN CEIL(fee_rate / 100.0) * 100.0
        ...
        GROUP BY bucket;
      `;
            const [outRows] = await database_1.default.query(outQuery, [node_public_key, node_public_key, node_public_key, node_public_key]);
            return {
            ...
            logger_1.default.err(`Cannot get node fee distribution for ${node_public_key}. Reason: ${(e instanceof Error ? e.message : e)}`);
            throw e;
        }
    }
    async $getAllNodes() {
        try {
            ...
            logger_1.default.err('$getAllNodes error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getNodeStats(public_key) {
        try {
            const query = `
        ...
        ORDER BY added DESC
      `;
            const [rows] = await database_1.default.query(query, [public_key]);
            return rows;
        }
        catch (e) {
            logger_1.default.err('$getNodeStats error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getTopCapacityNodes(full) {
        try {
            let rows;
            let query;
            if (full === false) {
          ...
          LIMIT 6
        `;
                [rows] = await database_1.default.query(query);
            }
            else {
                query = `
          SELECT nodes.public_key AS publicKey, IF(nodes.alias = '', SUBSTRING(nodes.public_key, 1, 20), alias) as alias,
        ...
        `;
            ...
            logger_1.default.err('$getTopCapacityNodes error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getTopChannelsNodes(full) {
        try {
            let rows;
            let query;
            if (full === false) {
        ...
        `;
            ...
            logger_1.default.err('$getTopChannelsNodes error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getOldestNodes(full) {
        try {
            let [rows] = await database_1.default.query('SELECT UNIX_TIMESTAMP(MAX(added)) as maxAdded FROM node_stats');
    ...
    async $searchNodeByPublicKeyOrAlias(search) {
        try {
            ...
            const [rows] = await database_1.default.query(query, [publicKeySearch, aliasSearch]);
            return rows;
        }
        catch (e) {
            logger_1.default.err('$searchNodeByPublicKeyOrAlias error: ' + (e instanceof Error ? e.message : e));
            throw e;
        }
    }
    async $getNodesISPRanking() {
        try {
            let query = '';
      ...
      `;
            const [channelsIsp] = await database_1.default.query(query);
            ...
            const [totalCapacity] = await database_1.default.query(query);
      ...
      `;
            const [clearnetCapacity] = await database_1.default.query(query);
      ...
      `;
            const [torCapacity] = await database_1.default.query(query);
            ...
            logger_1.default.err(`Cannot get LN ISP ranking. Reason: ${e instanceof Error ? e.message : e}`);
            throw e;
        }
    }
    async $getNodesPerCountry(countryId) {
        try {
            const query = `
        ...
        ORDER BY capacity DESC
      `;
            const [rows] = await database_1.default.query(query, [countryId]);
            for (let i = 0; i < rows.length; ++i) {
                rows[i].country = JSON.parse(rows[i].country);
                rows[i].city = JSON.parse(rows[i].city);
            ...
            logger_1.default.err(`Cannot get nodes for country id ${countryId}. Reason: ${e instanceof Error ? e.message : e}`);
            throw e;
        }
    }
    async $getNodesPerISP(ISPId) {
        try {
            let query = `
      ...
      `;
            ...
            const [rows] = await database_1.default.query(query, [IPSIds, IPSIds]);
        ...
        ORDER BY capacity DESC
      `;
            const [rows2] = await database_1.default.query(query, [Object.keys(nodes)]);
            ...
            logger_1.default.err(`Cannot get nodes for ISP id ${ISPId}. Reason: ${e instanceof Error ? e.message : e}`);
            throw e;
        }
    }
    async $getNodesCountries() {
        try {
      ...
      `;
            const [nodesCountPerCountry] = await database_1.default.query(query);
            query = `SELECT COUNT(*) as total FROM nodes WHERE country_id IS NOT NULL`;
            const [nodesWithAS] = await database_1.default.query(query);
            ...
            logger_1.default.err(`Cannot get nodes grouped by AS. Reason: ${e instanceof Error ? e.message : e}`);
            throw e;
        }
    }
    /**
    ...
    async $saveNode(node) {
      ...
      `;
            ...
            logger_1.default.err('$saveNode() error: ' + (e instanceof Error ? e.message : e));
        }
    }
    /**
     * Update node sockets
     */
    async $updateNodeSockets(publicKey, sockets) {
        const uniqueAddr = [...new Set(sockets.map(a => a.addr))];
        const formattedSockets = (uniqueAddr.join(',')) ?? '';
        try {
            await database_1.default.query(`UPDATE nodes SET sockets = ? WHERE public_key = ?`, [formattedSockets, publicKey]);
        }
        catch (e) {
            logger_1.default.err(`Cannot update node sockets for ${publicKey}. Reason: ${e instanceof Error ? e.message : e}`);
        }
    }
    /**
     ...
     * @asyncSafe
     */
    async $setNodesInactive(graphNodesPubkeys) {
      ...
      `);
            if (result[0].changedRows ?? 0 > 0) {
                logger_1.default.debug(`Marked ${result[0].changedRows} nodes as inactive because they are not in the graph`, logger_1.default.tags.ln);
            }
        }
        catch (e) {
            logger_1.default.err('$setNodesInactive() error: ' + (e instanceof Error ? e.message : e));
        }
    }
    aliasToSearchText(str) {
        return str.replace(/[-_.]/g, ' ').replace(/[^a-zA-Z0-9 ]/g, '');
    }
}
exports.default = new NodesApi();
