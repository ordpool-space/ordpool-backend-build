...
class Audit {
    auditBlock(height, transactions, projectedBlocks, mempool) {
        if (!projectedBlocks?.[0]?.transactionIds || !mempool) {
            return { unseen: [], censored: [], added: [], prioritized: [], fresh: [], sigop: [], fullrbf: [], accelerated: [], matchRate: 100, similarity: 1 };
        }
        const matches = []; // present in both mined block and template
        const added = []; // present in mined block, not in template
        const unseen = []; // present in the mined block, not in our mempool
        let prioritized = []; // higher in the block than would be expected by in-band feerate alone
        let deprioritized = []; // lower in the block than would be expected by in-band feerate alone
        const fresh = []; // missing, but firstSeen or lastBoosted within PROPAGATION_MARGIN
        const rbf = []; // either missing or present, and either part of a full-rbf replacement, or a conflict with the mined block
        const accelerated = []; // prioritized by the mempool accelerator
        ...
        for (const txid of projectedBlocks[0].transactionIds) {
            if (!inBlock[txid]) {
                ...
                if (rbf_cache_1.default.has(txid) && (rbf_cache_1.default.isFullRbf(txid) || rbf_cache_1.default.anyInSameTree(txid, (tx) => inBlock[tx.txid]))) {
                    rbf.push(txid);
                }
        ...
        while (projectedBlocks[blockIndex] && failures < 500) {
            ...
            if (tx) {
                ...
                logger_1.default.warn('projected transaction missing from mempool cache');
            }
            index++;
        ...
        for (const tx of transactions) {
            if (inTemplate[tx.txid]) {
                ...
                if (rbf_cache_1.default.has(tx.txid)) {
                    rbf.push(tx.txid);
                    if (!mempool[tx.txid] && !rbf_cache_1.default.getReplacedBy(tx.txid)) {
                        unseen.push(tx.txid);
                    }
                }
                else {
        ...
        ({ prioritized, deprioritized } = transaction_utils_1.default.identifyPrioritizedTransactions(transactions, 'effectiveFeePerVsize'));
        // transactions missing from near the end of our template are probably not being censored
        let overflowWeightRemaining = overflowWeight - (config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS - totalWeight);
