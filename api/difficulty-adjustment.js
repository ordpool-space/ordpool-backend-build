...
exports.calcDifficultyAdjustment = exports.calcBitsDifference = void 0;
 ...
 */
function calcBitsDifference(oldBits, newBits) {
    // Must be
    // - integer
...
exports.calcBitsDifference = calcBitsDifference;
function calcDifficultyAdjustment(DATime, quarterEpochTime, nowSeconds, blockHeight, previousRetarget, network, latestBlockTimestamp) {
...
exports.calcDifficultyAdjustment = calcDifficultyAdjustment;
class DifficultyAdjustmentApi {
    getDifficultyAdjustment() {
        const DATime = blocks_1.default.getLastDifficultyAdjustmentTime();
        const previousRetarget = blocks_1.default.getPreviousDifficultyRetarget();
        const blockHeight = blocks_1.default.getCurrentBlockHeight();
        const blocksCache = blocks_1.default.getBlocks();
        const latestBlock = blocksCache[blocksCache.length - 1];
        if (!latestBlock) {
            return null;
        }
        const nowSeconds = Math.floor(new Date().getTime() / 1000);
        const quarterEpochBlockTime = blocks_1.default.getQuarterEpochBlockTime();
        return calcDifficultyAdjustment(DATime, quarterEpochBlockTime, nowSeconds, blockHeight, previousRetarget, config_1.default.MEMPOOL.NETWORK, latestBlock.timestamp);
    }
}
exports.default = new DifficultyAdjustmentApi();
