...
class Common {
    ...
    static isLiquid() {
        return config_1.default?.MEMPOOL?.NETWORK === 'liquid' || config_1.default?.MEMPOOL?.NETWORK === 'liquidtestnet';
    }
    ...
    static isNonStandardAnchor(vin, height) {
        if (height != null
    ...
    static isStandardEphemeralDust(tx, height) {
        if (tx.fee === 0
            && (height == null || (this.EPHEMERAL_DUST_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]
                && height >= this.EPHEMERAL_DUST_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]))) {
            return true;
        }
        return false;
    }
    ...
    static isStandardOpReturn(bytes, outputs, height) {
        if ((height == null || (this.OP_RETURN_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]
            && height >= this.OP_RETURN_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK])) // limits lifted
            || // OR
    ...
    static isNonStandardLegacySigops(tx, height) {
        if (height == null || (this.LEGACY_SIGOPS_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]
            && height >= this.LEGACY_SIGOPS_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK])) {
            if (!transaction_utils_1.default.checkSigopsBIP54(tx, MAX_TX_LEGACY_SIGOPS)) {
                return true;
            }
        }
        return false;
    ...
    static inputIsMaybeInscription(vin) {
                    ...
                    case 'v1_p2tr':
                        {
                            ...
                            if (vin.witness?.length) {
                                ...
                                const hasAnnex = vin.witness.length > 1 && vin.witness[vin.witness.length - 1].startsWith('50');
                                if (hasAnnex) {
                                    flags |= mempool_interfaces_1.TransactionFlags.annex;
                                }
                            }
                        }
                        break;
    ...
    static indexingEnabled() {
        return (['mainnet', 'testnet', 'signet', 'testnet4', 'regtest'].includes(config_1.default.MEMPOOL.NETWORK) &&
    ...
    static findSocketNetwork(addr) {
        ...
        else if (addr.indexOf('ipv6') !== -1 || (config_1.default.LIGHTNING.BACKEND === 'lnd' && url.indexOf(']:'))) {
    ...
    static calcEffectiveFeeStatistics(transactions) {
        ...
        let weightCount = config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS - totalWeight;
        let medianFee = 0;
        let medianWeight = 0;
        ...
        const minFee = Math.min(Common.getNthPercentile(1, sortedTxs).rate, nonCoinbaseTransactions.slice(Math.ceil(nonCoinbaseTransactions.length * 49 / 50)).reduce((min, tx) => { return Math.min(min, tx.effectiveFeePerVsize || ((tx.fee || 0) / (tx.weight / 4))); }, Infinity));
        ...
        const maxFee = Math.max(Common.getNthPercentile(99, sortedTxs).rate, nonCoinbaseTransactions.slice(0, nonCoinbaseTransactions.length / 50).reduce((max, tx) => { return Math.max(max, tx.effectiveFeePerVsize || ((tx.fee || 0) / (tx.weight / 4))); }, 0));
        return {
    ...
    static getTransactionFromRequest(req, form) {
        const rawTx = typeof req.body === 'object' && form
