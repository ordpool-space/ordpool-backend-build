...
var CacheOp;
(function (CacheOp) {
    CacheOp[CacheOp["Remove"] = 0] = "Remove";
    CacheOp[CacheOp["Add"] = 1] = "Add";
    CacheOp[CacheOp["Change"] = 2] = "Change";
})(CacheOp || (CacheOp = {}));
/**
...
class RbfCache {
    replacedBy = new Map();
    replaces = new Map();
    rbfTrees = new Map(); // sequences of consecutive replacements
    dirtyTrees = new Set();
    treeMap = new Map(); // map of txids to sequence ids
    txs = new Map();
    expiring = new Map();
    cacheQueue = [];
    evictionCount = 0;
    ...
    add(replaced, newTxExtended) {
        ...
        const newTx = common_1.Common.stripTransaction(newTxExtended);
        ...
        for (const replacedTxExtended of replaced) {
            const replacedTx = common_1.Common.stripTransaction(replacedTxExtended);
    ...
    getRbfTrees(onlyFullRbf, after) {
        const limit = 25;
        const trees = [];
        const used = new Set();
        const replacements = Array.from(this.replacedBy).reverse();
    ...
    cleanup() {
        ...
        logger_1.default.debug(`rbf cache contains ${this.txs.size} txs, ${this.rbfTrees.size} trees, ${this.expiring.size} due to expire (${this.evictionCount} newly expired)`);
    ...
    async updateCache() {
        ...
        for (const e of this.cacheQueue) {
            if (e.op === CacheOp.Add || e.op === CacheOp.Change) {
                ...
                if (value != null) {
                    await redis_cache_1.default.$setRbfEntry(e.type, e.txid, value);
                }
            }
            else if (e.op === CacheOp.Remove) {
                await redis_cache_1.default.$removeRbfEntry(e.type, e.txid);
            }
        }
        this.cacheQueue = [];
    }
    dump() {
        const trees = Array.from(this.rbfTrees.values()).map((tree) => { return this.exportTree(tree); });
        return {
    ...
    /** @asyncSafe */
    async load({ txs, trees, expiring, mempool, spendMap }) {
        try {
            txs.forEach(txEntry => {
            ...
            logger_1.default.debug(`loaded ${txs.length} txs, ${trees.length} trees into rbf cache, ${expiring.length} due to expire, ${this.staleCount} were stale`);
            this.cleanup();
        }
        catch (e) {
            logger_1.default.err('failed to restore RBF cache: ' + (e instanceof Error ? e.message : e));
        }
    }
    exportTree(tree, deflated = null) {
        if (!deflated) {
            deflated = {
                root: tree.tx.txid,
            };
    ...
    importTree(mempool, root, txid, deflated, txs, mined = false) {
        ...
        const strippedTx = common_1.Common.stripTransaction(tx);
    ...
    async checkTrees() {
        ...
        if (config_1.default.MEMPOOL.BACKEND === 'esplora') {
            ...
            const sliceLength = Math.ceil(config_1.default.ESPLORA.BATCH_QUERY_BASE_SIZE / 40);
            for (let i = 0; i < Math.ceil(txids.length / sliceLength); i++) {
                ...
                try {
                    const txs = await bitcoin_api_factory_1.default.$getRawTransactions(slice);
                    processTxs(txs);
                    logger_1.default.debug(`fetched and processed ${processedCount} of ${txids.length} cached rbf transactions (${(processedCount / txids.length * 100).toFixed(2)}%)`);
                }
                catch (err) {
                    logger_1.default.err(`failed to fetch or process ${slice.length} cached rbf transactions`);
                }
            }
        }
        else {
            ...
            for (const txid of txids) {
                try {
                    const tx = await bitcoin_api_factory_1.default.$getRawTransaction(txid, true, false);
                    txs.push(tx);
                }
                catch (err) {
