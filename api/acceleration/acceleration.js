...
class AccelerationCosts {
     ...
     * @param verboseBlock
     */
    calculateBoostRate(accelerations, blockTxs) {
        ...
        const template = (0, mini_miner_1.makeBlockTemplate)(blockTxs, accelerations, 1, Infinity, Infinity);
     ...
     * @param medianFeeRate
     */
    getAccelerationInfo(tx, targetFeeRate, transactions) {
        // Get same-block transaction ancestors
        const allRelatives = (0, mini_miner_1.getSameBlockRelatives)(tx, transactions);
        const relativesMap = (0, mini_miner_1.initializeRelatives)(allRelatives);
        const rootTx = relativesMap.get(tx.txid);
     ...
     * @param ancestors
     */
    calculateAccelerationAncestors(tx, relatives, targetFeeRate) {
        ...
        relatives.forEach(entry => (0, mini_miner_1.setAncestorScores)(entry));
        // Sort by descending ancestor score
        let sortedRelatives = Array.from(relatives.values()).sort(mini_miner_1.mempoolComparator);
        let includedInCluster = null;
        ...
        while (sortedRelatives.length && sortedRelatives[0].score && sortedRelatives[0].score >= targetFeeRate && maxIterations > 0) {
            ...
            if (best) {
                ...
                (0, mini_miner_1.removeAncestors)(cluster, relatives);
                // re-sort
                sortedRelatives = Array.from(relatives.values()).sort(mini_miner_1.mempoolComparator);
            }
        }
        // sanity check for infinite loops / too many ancestors (should never happen)
        if (maxIterations <= 0) {
            logger_1.default.warn(`acceleration dependency calculation failed: calculateAccelerationAncestors loop exceeded ${MAX_RELATIVE_GRAPH_SIZE} iterations, unable to proceed`);
...
exports.default = new AccelerationCosts;
